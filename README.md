...
| `10_trigeminal.R` | the trigeminal ganglion analysis: genotype within age, the adult three-group comparison, and a wild-type age positive control, restricted to the three panels |
| `run_all.R` | driver and provenance |

`07` precedes the figures because they read its verdicts. `09` and `10` are
analyses that nothing downstream consumes.

### The trigeminal ganglion

Every trigeminal group holds two libraries, below the minimum of three at
which `03` makes expression-status calls, so `10_trigeminal.R` is a
conventional replicate-aware analysis. It uses the corneal conventions where
they carry over: the same pre-filter, one factor combining age, genotype and
transparency, unshrunk fold changes, the same equivalence bound, and
Benjamini–Hochberg correction within primary panel and contrast. It departs
...
the count matrix was regenerated with fragment counting, and that fitted each
age separately, shrank fold changes and took adjusted P values from a
genome-wide correction with independent filtering. The new
step reads the current matrix and applies the corneal conventions listed under
*The trigeminal ganglion*. Trigeminal numbers produced by the standalone script
are superseded by this step's output. No corneal number changed.
